...
double* opt_1x4_7(int row_a, int col_a, int row_b, int col_b)
{
    ...
    for ( j=0; j<col_b; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
        for ( i=0; i<row_a; i+=1 ){        /* Loop over the rows of C */
         ...
         and the jth column of B */
            ...
            AddDot1x4_7(col_a, &A( i,0 ), lda, &B( 0,j ), ldb, &C( i,j ), ldc );
        }
    }
...
//    std::cout << " opt_1x4_7: " << std::fixed  << duration << " ms" << std::endl;
    free(a);
    free(b);
    return c;
}
