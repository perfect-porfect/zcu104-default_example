#ifndef OPT_4X4_7_H
#define OPT_4X4_7_H

#include "common.h"

double* opt_4x4_7(int row_a, int col_a, int row_b, int col_b)
{
    double *a =  malloc(row_a* col_a * sizeof(double));
    double *b =  malloc(row_b* col_b * sizeof(double));
    double *c =  malloc(row_a* col_b * sizeof(double));
    int lda = row_a;
    int ldb = row_b;
    int ldc = row_a;
    int i, j;

    for (int i = 0; i < row_a * col_a; i++)
        a[i] = (double) i+1;

    for (int i = 0 ; i < row_a * col_b; i++)
        b[i] = (double)-i-1;

    for (int i = 0 ; i < row_a * col_b; i++)
        c[i] = 0.0;

//    auto start = std::chrono::high_resolution_clock::now();
	XTime tStart, tEnd;

	XTime_GetTime(&tStart);
    for ( j=0; j<col_b; j+=4 ){        /* Loop over the columns of C, unrolled by 4 */
        for ( i=0; i<row_a; i+=4 ){        /* Loop over the rows of C */
            /* Update the C( i,j ) with the inner product of the ith row of A
         and the jth column of B */

            AddDot4x4_7(col_a, &A( i,0 ), lda, &B( 0,j ), ldb, &C( i,j ), ldc );
        }
    }

	XTime_GetTime(&tEnd);
	double elapsed = 1.0 * (tEnd - tStart) / (COUNTS_PER_SECOND/1000000);
	printf("opt4x4_7: time : %.2f us.\n", elapsed);
//    auto end = std::chrono::high_resolution_clock::now();

//    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>( end - start ).count();
//    std::cout << " opt4x4_7: " << std::fixed  << duration << " ms" << std::endl;
    free(a);
    free(b);
    return c;
}

#endif // OPT_4X4_7_H
